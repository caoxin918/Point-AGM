...
class Point2VecClassification(pl.LightningModule):
    def __init__(
        ...
        tokenizer_unfreeze_epoch: int = 1,
        positional_encoding_unfreeze_epoch: int = 1,
        encoder_dim: int = 384,
        ...
        pretrained_ckpt_path: str or None = None,
    ...
    def training_step(
        ...
        label = label.to(torch.int64)  # 针对ModelNetDataset所加
        ...
        # T-SANE
        # self.x_list_val.append(logits.cpu())
        # self.label_list_val.append(label.cpu())
        # T-SANE
        return loss

    # T-SANE
    # def trainging_epoch_end(self, outputs) -> None:
    #     x = torch.cat(self.x_list_val, dim=0)  # (N, 768)
    #     y = torch.cat(self.label_list_val, dim=0)  # (N,)
    #     self.x_list_val = []
    #     self.label_list_val = []
    #     import numpy as np
    #     np.save("experiments/feature_cls_val_modelnet10.npy", x.detach().cpu().numpy())
    #     np.save("experiments/label_cls_val_modelnet10.npy", y.detach().cpu().numpy())
    # T-SANE
    ...
    # 	svm_validation: Dict[str, pl.LightningDataModule] = self.hparams.svm_validation  # type: ignore
    # 	for dataset_name, datamodule in svm_validation.items():
    # 	    self.tSNE_visualization(datamodule)


    def tSNE_visualization(self, datamodule: pl.LightningDataModule) -> Tuple[float, float]:
        ...
        def xy(
            ...
            for (points, label) in iter(dataloader):
                ...
                x = self.encoder(embeddings, pos).last_hidden_state
        ...
        catfile = os.path.join('./data/modelnet10_hdf5_2048', 'shape_names.txt')
        cat = [line.rstrip() for line in open(catfile)]
        classes = dict(zip(range(len(cat)), cat))
        y_train = pd.DataFrame({'label': pd.Series(list(y_train))})
        y_train = y_train.replace(classes)
        fig, ax = plt.subplots(figsize=(15, 12), dpi=1080)
        groups = pd.DataFrame(transformed, columns=['x', 'y']).assign(category=y_train).groupby('category')
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        for name, points in groups:
            ax.scatter(points.x, points.y, s=40., label=name)
        box = ax.get_position()
        ...
        ax.legend(loc='center left', bbox_to_anchor=(1.02, 0.5), fontsize=26)
        plt.savefig('./tsne1080_pre.png', dpi=1080)
        plt.show()
